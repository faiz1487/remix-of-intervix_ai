...
import {
  Sparkles, Loader2, Upload, Briefcase, Target, TrendingUp,
...
const NaukriOptimizerPage = () => {
  ...
  const [analysis, setAnalysis] = useState<NaukriAnalysis | null>(null);
  ...
  const handleAnalyze = async () => {
    if (!profileText.trim() && !imageDataUrl) {
      toast.error("Upload a screenshot, resume, or paste your Naukri profile content.");
      return;
    }
    setIsAnalyzing(true);
    setAnalysis(null);
    try {
      const { data, error } = await supabase.functions.invoke("naukri-optimizer", {
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <ModuleHeader
        ...
        description="ATS + recruiter visibility + senior-level branding"
      ...
      <div className="flex-1 overflow-auto">
        <div className="max-w-6xl mx-auto p-4 md:p-6 space-y-6">
          ...
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} className="glass rounded-2xl p-6 space-y-5 shadow-glow">
            ...
            <div>
              <label className="block text-sm font-medium mb-2">📄 Naukri Profile / Resume Content</label>
              <div className="flex flex-wrap gap-2 mb-2">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => imageRef.current?.click()} className="rounded-xl text-xs">
                  <ImageIcon className="w-3.5 h-3.5 mr-1" /> Upload Naukri Screenshot
                </Button>
                {imageDataUrl && (
              ...
              <textarea
                ...
                placeholder="Paste your Naukri headline, summary, skills, experience, and projects here..."
                rows={8}
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="space-y-5">
                ...
                <div className="glass rounded-2xl p-6 border border-primary/20 shadow-glow">
                  <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                    <div>
                      <p className="text-xs text-muted-foreground mb-1">Overall Recruiter Score</p>
                ...
                <div className="glass rounded-2xl p-5">
                  ...
                  <div className="space-y-3">
                    <div>
                      <p className="text-xs text-muted-foreground mb-1.5">Missing ATS Keywords</p>
                ...
                <Section title="Headline Optimization" icon={<Sparkles className="w-4 h-4 text-primary-foreground" />} defaultOpen>
                  ...
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">ATS Optimized:</b> {analysis.headline?.atsOptimized}</div>
                ...
                <Section title="Resume Headline" icon={<FileText className="w-4 h-4 text-primary-foreground" />}>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">ATS Optimized:</b> {analysis.resumeHeadline?.atsOptimized}</div>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Branding Statement:</b> {analysis.resumeHeadline?.brandingStatement}</div>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Recruiter-Focused:</b> {analysis.resumeHeadline?.recruiterFocused}</div>
                </Section>
                ...
                <Section title="Profile Summary" icon={<FileText className="w-4 h-4 text-primary-foreground" />}>
                  ...
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">ATS-Rich:</b> {analysis.summary?.atsRich}</div>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Production-Focused:</b> {analysis.summary?.productionFocused}</div>
                ...
                <Section title="Key Skills Optimization" icon={<Zap className="w-4 h-4 text-primary-foreground" />}>
                  ...
                  <div>
                    <b className="text-foreground">Recommended Order (top 10 matter most):</b>
                ...
                <Section title="ATS Keyword Analysis" icon={<Target className="w-4 h-4 text-primary-foreground" />}>
                ...
                <Section title="Salary & Notice Period Strategy" icon={<Briefcase className="w-4 h-4 text-primary-foreground" />}>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Salary Positioning:</b> {analysis.salaryNoticeStrategy?.salaryPositioning}</div>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Notice Period:</b> {analysis.salaryNoticeStrategy?.noticePeriod}</div>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Recruiter Filtering:</b> {analysis.salaryNoticeStrategy?.recruiterFiltering}</div>
                </Section>
                ...
                <Section title="Recruiter Visibility Hacks" icon={<TrendingUp className="w-4 h-4 text-primary-foreground" />}>
                  ...
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Best Time to Update:</b> {analysis.recruiterVisibility?.bestTimeToUpdate}</div>
                  <div className="bg-muted/30 rounded-xl p-3"><b className="text-primary">Application Strategy:</b> {analysis.recruiterVisibility?.applicationStrategy}</div>
                </Section>

                <Section title="LinkedIn + GitHub + Portfolio" icon={<Sparkles className="w-4 h-4 text-primary-foreground" />}>
                  <div><b className="text-foreground">LinkedIn:</b><ul className="mt-1 space-y-1">{analysis.linkedinGithub?.linkedin?.map((s: string, i: number) => <li key={i}>• {s}</li>)}</ul></div>
                  <div><b className="text-foreground">GitHub:</b><ul className="mt-1 space-y-1">{analysis.linkedinGithub?.github?.map((s: string, i: number) => <li key={i}>• {s}</li>)}</ul></div>
                  <div><b className="text-foreground">Portfolio:</b><ul className="mt-1 space-y-1">{analysis.linkedinGithub?.portfolio?.map((s: string, i: number) => <li key={i}>• {s}</li>)}</ul></div>
                </Section>
